...
async function main() {
  ...
  const catByName = Object.fromEntries(
    (await prisma.serviceCategory.findMany()).map((c: any) => [c.name, c])
  );
  ...
  for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
    ...
    for (const emp of allEmployees) {
      // Skip admin user
      if (emp.firstName === 'Admin') continue;
      
      // Some employees work different days
      if (emp.firstName === 'Sarah' && [1, 3, 5].includes(shiftDate.getDay())) continue; // Mon, Wed, Fri off
      if (emp.firstName === 'Mike' && [2, 4].includes(shiftDate.getDay())) continue; // Tue, Thu off
